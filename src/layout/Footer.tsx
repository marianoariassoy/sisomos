  ...
  return (
    <footer>
      <div className='py-8 px-6 shadow-lg flex flex-col gap-y-6'>
        <nav className='text-secondary flex justify-center font-semibold [&>a:last-child]:border-0'>
          {menu.map((item, index) => (
            <a
              key={index}
              href=''
              className='hover:opacity-50 transition-opacity border-r border-gray-600 px-3'
            >
              {item.title}
            </a>
          ))}
